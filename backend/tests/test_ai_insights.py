...
def test_insights_degrades_when_the_model_fails(auth_client, monkeypatch, caplog):
    _use_llm(monkeypatch)
    _two_hours_of_closed_work()

    def boom(system: str, user_message: str):
        raise TimeoutError("upstream timed out token=secret-upstream-value")
    ...
    body = response.json()
    assert body["ai_ok"] is False
    assert body["ai_error"] == "LLM service unavailable"
    assert "secret-upstream-value" not in str(body)
