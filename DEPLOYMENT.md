...
**Статус: READY** — домашний сервер `server-1`, рабочий каталог
`/home/kravtandr/proj/AI_Kanban`. Host-специфичная конфигурация находится в
`docker-compose.override.yml`; заполненный `.env` хранится только на сервере.
HTTPS-модель принята в ADR-0006 и описана в разделе «HTTPS в LAN» ниже.

Деплой выполняется только вручную владельцем или по его явному запросу; setup и CI
никогда не деплоят. Пред-условие любого деплоя: `make verify` (PASS) на точном
деплоящемся состоянии рабочей копии.
...
- Сервис `backup` запускает `deploy/backup.sh`: ежедневный pg_dump во временный файл,
  gzip и атомарная публикация в `./backups/`; ротация 14 дней после успешного бэкапа.
